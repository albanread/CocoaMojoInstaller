...
struct Layout {
    ...
    /// The payload's flavor, written by make-payload.sh. An absent file is
    /// the full flavor -- every payload cut before flavors existed carries
    /// the IDE, so absence has to mean what those payloads meant.
    ///
    ///     full            the toolchain and Roast.app, host-native code
    ///     toolchain-v3    the toolchain alone, compiled for x86-64-v3 --
    ///                     the portable flavor, prepared for the MacBook
    ///                     after a native dist died there in the first
    ///                     instruction block it executed
    var payloadFlavor: String {
        let f = payload.appendingPathComponent("FLAVOR")
    ...
    var toolchainOnly: Bool { payloadFlavor.hasPrefix("toolchain") }
...
final class Operations {
    ...
    /// The version `current` points at, or nil. The window uses this to
    /// say "already installed" instead of making the person find out.
    ...
    /// One integer sysctl flag, or -1. hw.optional.* is how a Mac states
    /// its ISA in public.
    private func sysctlFlag(_ name: String) -> Int {
        var v: Int32 = 0
        var n = MemoryLayout<Int32>.size
        guard sysctlbyname(name, &v, &n, nil, 0) == 0 else { return -1 }
        return Int(v)
    }

    /// Run one installed binary and demand it lives. This is the whole
    /// point of the flavored installer: the failure it guards against is a
    /// binary that copies cleanly and dies on its first instruction block,
    /// which a person then debugs as a corrupt transfer. Executing
    /// `cocoamojo-compiler --version` proves the ISA contract on THIS Mac,
    /// at install time, with words.
    private func proveExecutable(_ landed: URL) throws {
        let bin = landed.appendingPathComponent("bin/cocoamojo-compiler")
        ...
        p.standardError = out
        do { try p.run() } catch {
            throw Failure.reason("could not launch \(bin.path): \(error)")
        }
        p.waitUntilExit()
        if p.terminationReason == .uncaughtSignal {
            let sig = p.terminationStatus
            var msg = "\(bin.lastPathComponent) died with signal \(sig) on "
                    + "its first run."
            if sig == SIGILL {
                msg += " SIGILL here means an instruction this CPU does not "
                     + "have -- a toolchain flavor built for a wider ISA "
                     + "(the native Xeon build carries AVX-512; this Mac "
                     + "has AVX2). The right download is the x86-64-v3 "
                     + "flavor."
            }
            throw Failure.reason(msg)
        }
        guard p.terminationStatus == 0 else {
            throw Failure.reason(
                "\(bin.lastPathComponent) exited \(p.terminationStatus) on "
              + "its first run; the toolchain did not prove itself")
        }
        let line = String(data: out.fileHandleForReading.readDataToEndOfFile(),
                          encoding: .utf8)?
            .split(separator: "\n").first.map(String.init) ?? "ok"
        say("  proof of execution on this Mac: \(line)")
    }
    ...
    func generateDatabase(into toolchain: URL,
        ...
        // The PAYLOAD's interpreter, always -- never the installed one and
        // never the machine's. The database describes an ABI, and which
        // Python computed it is part of the answer: /usr/bin/python3 is
        // whatever Xcode last shipped, and the installed copy may not exist
        // at all because installing it is the person's choice. The packed
        // one is the same version on every Mac, which is the only way two
        // installations can be expected to agree.
        let home = layout.payload
            .appendingPathComponent("CocoaMojo")
            .appendingPathComponent("Python/Python.framework/Versions/Current")
        let interpreter = home.appendingPathComponent("bin/python3")
        guard exists(interpreter) else {
            throw Failure.reason(
                "No Python in the payload at \(home.path) -- the image is"
                + " incomplete; the database is always built with the"
                + " interpreter that ships beside it")
        }
        ...
        // PYTHONHOME is the relocation contract: a copied framework still
        // carries its build prefix, and this one is running from wherever
    ...
    /// - Parameter python: install the bundled CPython. Optional because
    ///   47 MB is a real cost to someone who already has a Python they
    ///   like, and because the pieces that need one degrade rather than
    ///   break: the database generator falls back to `/usr/bin/python3`,
    ///   and Roast looks for a framework Python already on the machine.
    func install(python: Bool = true) throws {
        ...
        let flavor = layout.payloadFlavor
        say("Installing CocoaMojo \(version) (\(flavor)) into \(layout.root.path)")

        // The lesson this installer exists to keep: a dist can transfer
        // perfectly and still be unrunnable, dying on the first AVX-512
        // instruction with nothing but EXC_BAD_INSTRUCTION. So the flavor's
        // ISA contract is CHECKED, in words, before a byte is copied.
        if flavor.hasPrefix("toolchain-v3") || flavor.contains("x86-64-v3") {
            if sysctlFlag("hw.optional.avx2_0") != 1 {
                throw Failure.reason(
                    "This Mac does not report AVX2 (hw.optional.avx2_0), and "
                  + "the x86-64-v3 flavor requires it. No Intel Mac that can "
                  + "run current macOS lacks it, so this is most likely not "
                  + "the machine this installer was built for.")
            }
            if sysctlFlag("hw.optional.avx512f") == 1 {
                say("  this host also has AVX-512; the portable flavor runs "
                  + "here too, just without it")
            }
        }
        ...
        let landed = layout.toolchains.appendingPathComponent(version)
        try replace(layout.payloadToolchain, with: landed)
        say("  toolchain copied")
        try proveExecutable(landed)

        // Declining Python removes BOTH the framework and the wrapper that
        // reaches it. A bin/python3 pointing at nothing is worse than no
        // bin/python3: it is on the path where a command belongs, and it
        // fails only when someone tries to use it.
        if !python {
            for part in ["Python", "bin/python3"] {
                let victim = landed.appendingPathComponent(part)
                if exists(victim) { try? fm.removeItem(at: victim) }
            }
            say("  Python not installed (47 MB saved) -- Roast's Python"
                + " features will be off; run this again to add it")
        }
        ...
        // RELATIVE, deliberately. An absolute destination bakes
        // /Applications/Roast into the link, so moving or renaming the
        // folder leaves `current` pointing at a path that no longer
        // exists -- and `current` is what every lookup resolves through.
        // A relative link names its sibling and survives the move.
        try fm.createSymbolicLink(
            atPath: layout.current.path, withDestinationPath: version)
        say("  current -> \(version)")
        ...
        } else if layout.toolchainOnly {
            say("  toolchain flavor: no Roast.app in this payload, by design")
            say("  (the IDE flavor follows once Mojo can emit portable code)")
        } else {
