...
NOTARIZE=1
# Build everything and cut the image, but sign nothing. For a machine that
# has no Developer ID -- the Intel fork builds here and signs elsewhere --
# and for checking the payload without a round trip. The DMG it leaves is
# the same bytes the signed one is made from.
SIGN=1
# --flavor=toolchain-v3 cuts the portable, IDE-less image for the MacBook:
# skips the thin app (Mojo emits host-native code), stamps the payload's
# FLAVOR, and names the DMG for what it is.
FLAVOR="full"
for a in "$@"; do
  case "$a" in
...
case "$FLAVOR" in
  toolchain-v3) DMG="$OUT/MojoToolchain-v3-$VER.dmg" ;;
  *)            DMG="$OUT/Roast-$VER.dmg" ;;
esac

echo "== 1/8  the thin app =="
if [ "${FLAVOR}" != full ]; then
  echo "   skipped ($FLAVOR flavor ships no IDE)"
else
...
THIN=1 "$MOJOCOCOA/tools/make-app.sh" --no-dmg 2>&1 | tail -3 | sed 's/^/   /'
fi

echo "== 2/8  payload =="
FLAVOR="$FLAVOR" VERSION="$VER" PAYLOAD_DIR="$STAGE/payload" ./tools/make-payload.sh \
  | sed 's/^/   /'
...
[ -f "$MOJOCOCOA/tools/roast.icns" ] && {
  cp -f "$MOJOCOCOA/tools/roast.icns" "$APP/Contents/Resources/AppIcon.icns"
...
codesign --verify --deep --strict "$APP" && echo "   installer signed"
  ...
  echo "   skipped (--no-sign): the payload and the installer are unsigned"
fi
...
if [ "$SIGN" != 1 ]; then
  ...
  echo
  echo "$DMG"
  echo "  UNSIGNED and ready to sign. On a machine with the Developer ID:"
  echo "    codesign --force --timestamp --options runtime --sign \"<ID>\" \"Install Roast.app\""
  echo "    ./tools/sign-payload.sh <payload>   # then rebuild the image"
  exit 0
fi
